...
(* 	
	...
	Hur Toolbox    
	Version 0.5, compatible with Mathematica 11.0  	

	Revision date: November 8, 2017 
*)  
...
HurInitialize := ( 	
	HurGlobalRF = {Global`n}; HurGlobalDCM = List[RotationMatrix[0, {0, 0, 1}]]; HurGlobalMass={1}; HurGlobalInertia={{1,0,0,1,0,1}};
  ...
  HurGlobalListTriads = {0}; HurGlobalTriadsConversion = {0};
...
HurDot::usage="HurDot[v1_, v2_] is used to perform the dot product of the two vectors. v1 and v2 can be in different RFs. Ex) HurDot[v1, v2]";
...
HurGetLinearCOMVel::usage="HurGetLinearCOMVel[rf1_,rf2_] returns the angular velocity of the reference frame rf1 with respect to the world reference frame n using the triads of rf2. It uses skew symmetric matrix Sw such that Sw=R_dot x R'. Ex) w=HurGetAngularVel[b,d]";
HurGetLinearCOMAcc::usage="HurGetLinearCOMAcc[rf1_,rf2_] returns the angular acceleration of the reference frame rf1 with respect to the world reference frame n using the triads of rf2. It directly differentiate the angular velocity. Ex) w=HurGetAngularAcc[b,d]";
HurDefineCOMPos::usage="HurDefineCOMPos[rf_,v_]";
HurSetCOMVel::usage="HurSetCOMVel[rf_,v_]";
HurSetAngularVel::usage="HurSetAngularVel[rf_,w_]";
HurSetAngularAcc::usage="HurSetAngularAcc[rf_,alpha_]";
...
HurKinematics::usage="HurKinematics[]";
...
HurDefineDCM[rf_, dcm_] := (HurGlobalDCM[[ HurGetIndexGlobalRF[ rf ] ]] = dcm;
  nrf=Length[HurGlobalRF];
...
HurAppendRF2Coord[coord_, rf_] := Flatten[ List[coord,rf] ]
